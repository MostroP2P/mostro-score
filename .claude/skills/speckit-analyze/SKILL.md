...
Abort with an error message if any required file is missing (instruct the user to run missing prerequisite command). File existence alone is not sufficient: parse TASKS and confirm it contains at least one task entry in the required checklist format (`- [ ] T### ...`); if `tasks.md` exists but is empty or has no entries in that format, abort and instruct the user to run `/speckit-tasks` before proceeding. Only build coverage mappings once this structural validation passes.
