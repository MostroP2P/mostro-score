//! The persisted-configuration file's TOML schema, semantic validation, and
//! warn-and-ignore degradation. `load_config_file` collapses all 4 "no config value to
//! use" cases (absent, I/O error, parse error, semantic error) into `None`, warning to an
//! injected `err` writer for the latter 3 only; a missing file stays silent.
...
/// The raw TOML schema. `deny_unknown_fields` rejects any other top-level key without an
/// additional hand-written check -- this surfaces as `toml::from_str`'s own deserialize
/// error (`LoadFailure::Parse`, not `LoadFailure::Semantic`), which is why that failure's
/// warning message stays generic rather than claiming invalid syntax.
...
/// The validated, semantically-typed result of a successfully loaded configuration file.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ConfigFile {
    /// Validated at load time with the same check `nostr_sdk::PublicKey::parse` performs
    /// for an explicit `--pubkey` flag, so a value reaching this field is always
    /// well-formed -- kept as the original `String`, not a parsed `PublicKey`, since the
    /// flag-based path already re-parses it at its own usage point.
    pub pubkey: Option<String>,
    pub relays: Option<Vec<String>>,
    pub format: Option<Format>,
    pub view: Option<Granularity>,
    /// `Some(true)` for `color = "always"`, `Some(false)` for `color = "never"`, `None`
    /// when absent (automatic).
    pub color_override: Option<bool>,
    /// An empty array is treated as absent, same rule as `relays`.
    pub sections: Option<SectionFilter>,
}

/// Every way a configuration file fails to supply a usable value, distinguished
/// internally so only the last three warn.
enum LoadFailure {
    NotFound,
    Io(std::io::Error),
    Parse(toml::de::Error),
...
/// Loads and validates the configuration file at `path`. Returns `None` for all 4 cases
/// (absent, I/O error, malformed TOML, semantically invalid value); warnings are written
/// to `err` for every case except "absent", which stays silent. Application is
/// all-or-nothing: a semantically invalid value invalidates the entire file, never
/// applying only its valid keys.
pub fn load_config_file(path: &Path, err: &mut impl std::io::Write) -> Option<ConfigFile> {
    match load_config_file_inner(path) {
        ...
        Err(LoadFailure::Parse(parse_error)) => {
            // Covers both a genuine TOML syntax error and `deny_unknown_fields` rejecting
            // an unrecognized key, so the message stays generic rather than claiming
            // invalid syntax.
            let _ = writeln!(
                err,
...
fn load_config_file_inner(path: &Path) -> Result<ConfigFile, LoadFailure> {
    // `symlink_metadata` stats the path entry itself without following a symlink, so a
    // broken symlink (which must warn) is distinguished from genuine absence (which must
    // stay silent) even though both would otherwise fail `read_to_string` identically
...
mod tests {
    ...
    /// Same rule as `relays`: an empty array is absent, not an error and not "show
    /// nothing".
    #[test]
    ...
    /// A broken symlink must warn, distinct from the silent "genuinely absent" case,
    /// even though both fail `read_to_string` with the same `ErrorKind::NotFound`.
    #[test]
    #[cfg(unix)]
    ...
    /// One valid key plus one semantically invalid key invalidates the ENTIRE file --
    /// the valid key is NEVER silently applied alongside the bad one.
    #[test]
    ...
    /// Rejected via `deny_unknown_fields`, surfacing internally as a `toml::from_str`
    /// deserialize error rather than this module's own `validate()` step -- the
    /// observable behavior (ignore the whole file, warn) is identical either way.
    #[test]
    ...
    /// An empty array falls through the chain, not as "no relays to query" and not as
    /// an error.
    #[test]
