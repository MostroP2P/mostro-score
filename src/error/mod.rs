...
//! `Display` message (no stack traces, no raw errors on any user-facing path) and its
//! own exit code, mapped in `exit_code.rs`.
//!
...
#[derive(Debug, Error)]
pub enum AppError {
    /// A syntactically supplied `--pubkey` that fails format validation. Message text
    /// matches the JSON fatal-error envelope's `invalid_pubkey` entry, so the console
    /// and JSON paths say the same thing.
    #[error("Invalid public key format.")]
    InvalidPubkey,

    /// Every configured relay failed to connect. A single failed relay among several
    /// that succeeded is a warning, not this variant. Carries the list of relays that
    /// failed, needed to populate the JSON fatal-error envelope's `error.relays` field.
    #[error("None of the configured relays could be reached.")]
    RelaysUnreachable(Vec<crate::fetch::client::RelayConnectFailure>),

    /// A usage error detected by application code after argument parsing succeeds
    /// (e.g. `--force` without `--init-config`, `--since` after `--until`).
    #[error("{0}")]
    UsageError(String),

    /// A relay fetch that returns zero usable events across all four scoped kinds
    /// (dev-fee, order, dispute, instance-status) — nothing about this node can be
    /// reported. Distinct from a fetch that returns events with, say, zero successful
    /// orders: that is a valid, reportable state, not this variant.
    #[error(
