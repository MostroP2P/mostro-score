...
/// Returns `Result<(), AppError>` directly so every propagated error is already part of
/// the typed taxonomy — `AppError::Other`'s `#[from] Box<dyn Error>` covers anything
/// not already a more specific variant, so `main`'s error handling never needs to
...
pub async fn run<E: EventSource>(
    ...
    // outage (every configured relay failed) is fatal; one or more failures among
    // relays that did connect is a warning to `err`, not a failure.
    ...
    // Transient status, not report content — written directly as a plain `writeln!` to
    // `err`, matching the relay-warning loop above. `--quiet` suppresses this line and
    // the "Fetched N events" line below — both are transient status narration, never
    // report content, so they are the only two lines `options.quiet` affects; the
    // relay-warning loop above and the no-dev-fee-anchor warning below are diagnostic
    // facts, not transient narration, and are never suppressed.
    if !options.quiet {
        writeln!(
            err,
    ...
    // Captured once, here, and reused for both the future-event exclusion below and
    // the report's own "now": a single consistent instant for the whole run, not two
    // separate clock reads that could disagree if execution takes any measurable time.
    ...
    // Routes the four scoped kinds into their own buckets, then gates on whether any
    // of them yielded usable data at all. A node with, say, zero successful orders but
    // a usable dispute or instance-status event is still a valid, reportable state.
    ...
    // Computed once here and reused both for the exit-4 gate below and for this
    // node's dispute stats, rather than aggregated a second time from the same event
    // set.
    let dispute_aggregate = aggregate_dispute_events(partitioned.dispute_events);
    // Computed once here and reused both for the exit-4 gate below and for this
    // node's bond-policy signal, rather than aggregated a second time from the same
    // event set.
    ...
    // report's plain `days_active` figure (it explains *why* that figure is an estimate
    // rather than the primary dev-fee-anchored value), so it stays a stderr warning —
    // that information is covered by the report's own fetch and longevity sections.
    ...
    // `cli::options` has already resolved everything explicitly given. Two deferred
    // defaults are resolved here, now that both `grid_orders` and `report_generated_at`
    // are available: `--until` alone defers `since` to the node's earliest order (a
    // data-dependent value `cli::options` cannot know); `--since` alone defers `until`
    // to this same `report_generated_at` instant, deliberately, rather than an earlier
    // `now` `cli::options` would have had to read before any relay was even queried —
    // using that earlier instant here would let it silently diverge from the one used
    // for the future-event exclusion and the report's own `generated_at`. General
    // statistics (above) are computed over the node's full history, entirely
    ...
    // Fires when `--view`/`--since`/`--until` combine to force a daily grid over a
    // wide range. A diagnostic fact about the requested range, not transient status
    // narration, so it is never suppressed by `--quiet`, matching the relay-warning
    // loop above.
    ...
    // Assembles the complete 5-section report. Rendering it below may narrow the 4
    // filterable sections per `options.sections`; the assembled `Report` itself is
    // always complete.
    ...
    // Dispatches on the resolved `Format` instead of always rendering console —
    // `options.color_override` only ever affects `Format::Console`; the plain and
    // JSON renderers never look at color at all.
