...
async fn main() {
    ...
    // plain terminal-based default; the report-generating path's `error_render_format`/
    // `resolve_run_options` below consult `output_present` through
    // `resolve_context_default` instead.
    ...
    // `--init-config` short-circuits entirely, before the existing configuration file is
    // ever loaded -- loading it here would parse/validate a file `--init-config` is not
    // supposed to consult at all, surfacing a stray warning for a malformed file it is
    // about to overwrite anyway. Its own error format uses only the explicit `--format`
    // flag and automatic resolution, never a config-sourced value.
    if args.init_config {
    ...
    // resolved report — a config-sourced `format` has the same precedence an explicit
    ...
    // usage error, validated after format resolution so it renders through the resolved
    // `--format` (e.g. `--format json --force` still produces a JSON fatal envelope).
    if args.force {
        exit_with_error(
    ...
    // `--pubkey` is required for every report-generating invocation (the `--init-config`
    // short-circuit above already returned before this point), but not for
    // `--init-config` itself — so this check, not clap's native required-argument
    // handling, enforces it, still as a usage error. Precedence: explicit flag/
    // environment variable (already collapsed by clap), then the config file's `pubkey`
    // value (already validated as well-formed at config-load time), then this usage
    // error.
    ...
    // Invalid pubkey exits via AppError::InvalidPubkey, matching the JSON fatal-error
    // envelope's `invalid_pubkey` code.
    ...
    // a `RelayEventSource` is ever constructed, rather than letting a malformed URL
    // reach `Client::add_relay` and fold into a generic `AppError::RelaysUnreachable`.
    ...
    // wiring root, so `fetch::client` itself never depends on `report`. `options.quiet`
    // is already resolved by this point, so the progress indicator is suppressed from
    ...
    // `AppError::Other` via `AppError`'s existing `#[from] std::io::Error` conversion,
    // rendered through the same pre-relay `exit_with_error` pattern every other
    // pre-relay failure already uses.
    let mut stdout;
    let mut output_file;
    ...
    .await
    {
        ...
        // fails afterward, that leaves a stray empty file with no confirmation message,
        // which could be mistaken for real (empty) output. Removing it here keeps
        // failure and file-creation atomic from the user's point of view -- best-effort,
        // since the failure being reported already takes priority over a cleanup error.
    ...
    // matching the relay-failure warnings and the no-dev-fee-anchor warning in
    // `lib.rs::run`, neither of which `--quiet` suppresses either.
    if let Some(path) = args.output.as_ref() {
...
/// write failure from `render_fatal_error` rather than propagating it — the mapped exit
/// code must still apply even if the message can't be printed (e.g. a closed
/// stderr/stdout).
...
// The golden-baseline test lives in `tests/metrics_end_to_end.rs` as an integration test
// against `mostro_score::run()`. The invalid-`--pubkey` branch above stays pinned at
// binary level, asserted by `tests/error_handling.rs`.
