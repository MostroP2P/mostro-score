...
/// Longevity: `first_seen_at` is `Some` only via the primary dev-fee anchor path; the
/// fallback path (no dev-fee anchor, but at least one qualifying successful order)
/// reports `first_seen_at` as `None` since it has no dev-fee event to derive it from,
/// even though `days_active` is still computable in that case.
...
/// Computes longevity. Primary anchor is the oldest qualifying kind `8383`
/// dev-fee-payment event's `created_at`. When the node has none, falls back to the
/// elapsed time between its first qualifying successful order's `created_at` and `now`
/// — matching the primary path's own "elapsed time to now" semantic, rather than
/// spanning first order to *last* order (which would stop increasing at the last trade
/// and read `0` for a node with exactly one successful order). When neither anchor
/// exists, both fields are reported as not applicable.
...
/// Cumulative performance. Pure exposure of values already computed by
...
/// Liveness: last successful trade and elapsed time since it, both not applicable when
/// the node has zero successful orders, plus the rolling 7/30/90-day successful-trade
/// counts.
#[derive(Debug, Clone, Copy, PartialEq)]
pub struct Liveness {
...
/// Computes activity consistency. The window is exactly 30 UTC calendar days ending on
/// and including today, not a rolling 30*86400-second cutoff: computing the window's
/// boundary as a raw timestamp comparison (rather than aligning both ends to UTC
/// calendar-day indices first) includes an extra day whenever `now` isn't exactly at a
/// UTC day boundary, since `now - 30*86400` and `now` land on the same day-of-week
/// offset but span 31 distinct day indices, not 30.
pub fn compute_activity_consistency(timestamps: &[i64], now: i64) -> (usize, usize) {
    ...
    // Unique UTC calendar days with a trade within the 30-day window.
...
mod tests {
    ...
    /// exactly one successful order would always read `0`, and `days_active` would stop
    /// increasing after the node's last trade.
    #[test]
    ...
    /// Regression: the window is exactly 30 UTC calendar days ending on and including
    /// today, not a rolling 30*86400-second cutoff. A trade on the day exactly 30 days
    /// before today (the window's first day) must still count; a trade one day further
    /// back must not.
    #[test]
