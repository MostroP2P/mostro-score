...
/// `std_dev_trade_sats` follows that same empty-set rule. `coefficient_of_variation`
/// has its own, stricter not-applicable rule: `None` when fewer than 2 orders exist, or
/// when `median_trade_sats` is exactly `0`, since dividing by a zero median is
/// undefined regardless of sample size. `median_trade_sats` is `f64`, not `u64`: an
/// even-sized set's median is the average of its two middle values, which is
/// legitimately fractional (e.g. `[0, 1]` medians to `0.5`), and truncating it to an
/// integer would both misreport the value and corrupt `coefficient_of_variation`'s
/// division. `amt` comes from an untrusted relay event (`models::order`'s
/// `saturating_add` on the same field carries the identical caveat): converting it to
/// `f64` loses precision above 2^53, but that threshold is already over 4x the entire
/// 21M BTC supply in sats, so no realistic `amt` value is affected — only a
...
mod tests {
    ...
    /// An even-sized set whose middle two values average to a fraction (e.g. 0 and 1)
    /// must report that fraction exactly, not truncate it to an integer — truncating it
    /// would also corrupt `coefficient_of_variation`'s division (a truncated-to-zero
    /// median would wrongly report CV as not applicable instead of a real value).
    #[test]
